...
void main() {
  group('VocabularyListScreen', () {
    ...
    testWidgets('shows creation options modal', (WidgetTester tester) async {
      ...
      expect(
        find.text('Create an empty Text-to-Text vocabulary'),
        findsOneWidget,
      );
      expect(
        find.text('Upload a Text-to-Text vocabulary from a CSV File'),
        findsOneWidget,
      );
      expect(
        find.text('Upload an Image-to-Text vocabulary from a directory'),
        findsOneWidget,
      );
    });
    ...
    testWidgets('can navigate to vocabulary detail', (
      WidgetTester tester,
    ) async {
      SharedPreferences.setMockInitialValues({
        'vocabularies': [
...
Goodbye,مع السلامة''';
    ...
    test('throws error for empty CSV', () {
      expect(
        () => CsvParser.parseCsvFile('empty.csv', ''),
...
Hello,مرحبا''';
      ...
      expect(
        () => CsvParser.parseCsvFile('invalid.csv', csvContent),
        throwsA(isA<ArgumentError>()),
      );
    });
...
صباح الخير,בוקר טוב''';
      ...
      final bothRtlResult = CsvParser.parseCsvFile(
        'both_rtl_test.csv',
        bothRtlCsv,
      );
...
Hello,مرحبا''';
      ...
      final arabicVariantsResult = CsvParser.parseCsvFile(
        'arabic_variants_test.csv',
        arabicVariantsCsv,
      );
...
Hello,سلام''';
      ...
      final persianVariantsResult = CsvParser.parseCsvFile(
        'persian_variants_test.csv',
        persianVariantsCsv,
      );
...
Hello,سلام''';
      ...
      final farsiVariantsResult = CsvParser.parseCsvFile(
        'farsi_variants_test.csv',
        farsiVariantsCsv,
      );
